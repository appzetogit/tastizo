...
export async function registerFcmTokenForLoggedInUser() {
  try {
    ...
    if (!vapidKey) {
      console.warn("[FCM] No VAPID key. Set FIREBASE_VAPID_KEY in Admin → Environment Variables, or VITE_FIREBASE_VAPID_KEY in .env");
      return;
    }

    // Register our service worker so Firebase does not try to use the non-existent default path
    const registration = await navigator.serviceWorker.register(FCM_SW_PATH, { scope: FCM_SW_SCOPE });
    ...
    if (!token) {
      console.warn("[FCM] No FCM token from getToken. Ensure notification permission is granted and VAPID key is set.");
      return;
    }
    ...
    console.error("[FCM] Error during web FCM registration:", error?.message || error);
    if (error?.code === "messaging/permission-blocked") {
      console.warn("[FCM] User denied notification permission. Token will stay null until permission is granted.");
