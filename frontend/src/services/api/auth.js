...
const AUTH = {
  ...
  USER_GOOGLE_LOGIN: "/food/auth/user/google/login",
...
export function loginUserWithGoogle(idToken, fcmToken = null, platform = "web") {
  const token = String(idToken || "").trim();
  if (!token) {
    return Promise.reject(new Error("Google ID token is required"));
  }

  return apiClient.post(AUTH.USER_GOOGLE_LOGIN, {
    idToken: token,
    ...(fcmToken ? { fcmToken, platform } : {}),
  });
}
