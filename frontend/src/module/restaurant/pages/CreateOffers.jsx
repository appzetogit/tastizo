...
export default function CreateOffers() {
  ...
  // Offers data state for "Track offers" tab
  ...
  // Fetch offers when switching to "Track offers" tab
  useEffect(() => {
    if (activeTab !== "track-offers") return
    ...
    const fetchOffers = async () => {
      try {
        ...
        const apiOffers = response?.data?.data?.offers || response?.data?.offers || []
        setOffers(Array.isArray(apiOffers) ? apiOffers : [])
      } catch (error) {
        if (!isMounted) return
        console.error("Error fetching restaurant offers:", error)
        setOffers([])
        setOffersError(error?.response?.data?.message || "Failed to load offers")
      } finally {
        if (isMounted) {
  ...
  const filteredOffers = offers.filter((offer) => {
    ...
    const isUpcoming = start && start > now && status !== "cancelled"
    const isEnded = end && end < now
    const isInactiveStatus = ["paused", "expired", "cancelled"].includes(status)

    if (offerStatus === "active") {
      // Active = running offers (status active and within date range)
    ...
    if (offerStatus === "scheduled") {
      // Scheduled = upcoming offers (start date in future, not cancelled)
      return isUpcoming
    }

    if (offerStatus === "inactive") {
      // Inactive = paused / expired / cancelled OR end date in past
      return isInactiveStatus || isEnded
    }

    return true
  })

  // Simple aggregate metrics based on all offers
  const metrics = (() => {
    if (!offers || offers.length === 0) {
      return {
        grossSalesFromOffers: "₹0",
        ordersFromOffers: "0",
        discountGiven: "₹0",
    ...
    offers.forEach((offer) => {
      (offer.items || []).forEach((item) => {
        if (typeof item.originalPrice === "number" && typeof item.discountedPrice === "number") {
    ...
    const gross = totalOriginal > 0 ? totalOriginal : 0
    const discount = totalDiscount > 0 ? totalDiscount : 0
    const effectivePct = gross > 0 ? (discount / gross) * 100 : 0

    return {
      grossSalesFromOffers: `₹${gross.toFixed(0)}`,
      ordersFromOffers: `${totalItems}`,
      discountGiven: `₹${discount.toFixed(0)}`,
      effectiveDiscount: `${effectivePct.toFixed(1)}%`,
      // For now, approximate "menu to order" with effective discount as we don't have conversion data
      menuToOrder: `${effectivePct.toFixed(1)}%`,
    }
  })()
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg p-4 border border-gray-200">
              ...
              <div className="space-y-0 divide-y divide-gray-100">
                {[
                  { label: "Gross sales from offers", value: metrics.grossSalesFromOffers, change: "" },
                  { label: "Orders from offers", value: metrics.ordersFromOffers, change: "" },
                  { label: "Discount given", value: metrics.discountGiven, change: "" },
                  { label: "Effective discount", value: metrics.effectiveDiscount, change: "" },
                  { label: "Menu to order", value: metrics.menuToOrder, change: "" },
                ].map((metric, index) => (
            ...
            <div>
              ...
              {loadingOffers ? (
                ...
                <div className="bg-white rounded-lg p-8 border border-gray-200 text-center">
                  <h3 className="text-lg font-bold text-gray-900 mb-2">Could not load offers</h3>
                ...
                <div className="space-y-3">
                  {filteredOffers.map((offer) => {
                    const firstItem = (offer.items && offer.items[0]) || null
                    const title =
                      firstItem?.itemName ||
                      (offer.goalId === "grow-customers" ? "Grow your customer base" : "Offer")
                    const period =
                      offer.startDate || offer.endDate
                        ? `${offer.startDate ? new Date(offer.startDate).toLocaleDateString("en-IN", {
                            day: "2-digit",
                            month: "short",
                          }) : "Now"} – ${
                            offer.endDate
                    ...
                    return (
                      <div
                        key={offer._id}
                        className="bg-white rounded-lg p-4 border border-gray-200 flex items-center justify-between gap-3"
                      >
                        ...
                        <div className="flex flex-col items-end gap-1 shrink-0">
                          ...
                          {firstItem && typeof firstItem.discountPercentage === "number" && (
                            <span className="text-xs font-semibold text-green-600">
                              {firstItem.discountPercentage.toFixed(0)}% OFF
                            </span>
                          )}
                        </div>
                      </div>
