...
export default function TableBooking() {
    ...
    return (
        <AnimatedPage className="bg-slate-50 dark:bg-[#050505] min-h-screen pb-24">
            {/* Header */}
            <div className="bg-white dark:bg-[#111111] px-4 pt-4 pb-12 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <button onClick={() => navigate(-1)} className="p-2 -ml-2 mb-4 bg-white dark:bg-[#181818] shadow-sm rounded-full">
                        <ArrowLeft className="w-6 h-6 text-slate-900 dark:text-slate-100" />
                    </button>
                    <div className="text-center">
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Book a table</h1>
                        <p className="text-gray-500 dark:text-gray-300 font-medium">{restaurant.name}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="px-4 -mt-6 relative z-20 space-y-4">
                {/* Guest Selector */}
                <div className="bg-white dark:bg-[#111111] rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <span className="font-semibold text-gray-700 dark:text-gray-100">Select number of guests</span>
                    <div className="relative">
                        <select
                            value={selectedGuests}
                            onChange={(e) => setSelectedGuests(parseInt(e.target.value))}
                            className="appearance-none bg-slate-50 dark:bg-[#020617] border border-slate-200 dark:border-slate-700 rounded-lg py-2 pl-4 pr-10 font-bold text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-[#2B9C64]"
                        >
                ...
                {/* Date Selector */}
                <div className="bg-white dark:bg-[#111111] rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-800">
                    ...
                    <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                        {dates.map((date, idx) => (
                            ...
                            >
                                <span className={`text-xs font-bold uppercase tracking-wider ${selectedDate.toDateString() === date.toDateString() ? "text-[#2B9C64]" : "text-gray-400 dark:text-gray-500"
                                    }`}>
                                    {formatDate(date)}
                                </span>
                                <span className={`font-bold ${selectedDate.toDateString() === date.toDateString() ? "text-gray-900 dark:text-white" : "text-gray-500 dark:text-gray-400"
                                    }`}>
                                    {formatDayNum(date)}
                                </span>
                            </button>
                ...
                {/* Time Selector */}
                <div className="bg-white dark:bg-[#111111] rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-800 min-h-[400px]">
                    <h3 className="font-bold text-gray-800 dark:text-gray-100 mb-4 flex items-center gap-2">
                        ...
                        {["Lunch", "Dinner"].map(type => (
                            <button
                                ...
                                className={`flex-1 py-2.5 rounded-lg font-bold text-sm transition-all ${activeTimeOfDay === type
                                    ...
                                    : "text-gray-500 dark:text-gray-300 hover:text-[#2B9C64]"
                                    }`}
                            >
                                {type}
                            </button>
