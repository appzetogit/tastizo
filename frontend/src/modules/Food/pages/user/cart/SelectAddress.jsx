...
export default function SelectAddress() {
  ...
  const { addresses = [], addAddress, getDefaultAddress } = useProfile()
  const { location: currentLocation, selectSavedAddress } = useLocation()

  const from = routerLocation?.state?.from || "/user/cart"
  ...
  const onSave = async (e) => {
    ...
    try {
      if (selectedSuggestionId) {
        const selectedAddress = addresses.find((addr) => String(getAddressId(addr)) === String(selectedSuggestionId))
        if (selectedAddress) {
      ...
      if (!Number.isFinite(coords.latitude) || !Number.isFinite(coords.longitude)) {
        toast.error("Pick a location from the map flow first")
        navigate("/user/address-selector", { state: { from } })
        return
      }
      ...
      if (created) await selectSavedAddress(created)
