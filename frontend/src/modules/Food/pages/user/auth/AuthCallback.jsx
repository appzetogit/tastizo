      ...
      try {
        ...
        if (token) {
          try {
            ...
            // Save auth data
            setAuthData("user", token, user, refreshToken || null)
        ...
        <CardContent className="space-y-6 md:space-y-8 p-6 md:p-8 lg:p-10 pt-0 md:pt-0 lg:pt-0">
          {status === "loading" && (
            <div className="flex flex-col items-center justify-center py-8 md:py-12 space-y-4 md:space-y-6">
              <Loader2 className="h-12 w-12 md:h-16 md:w-16 text-[#2A9C64] animate-spin" />
          ...
          {status === "success" && (
            <div className="flex flex-col items-center justify-center py-8 md:py-12 space-y-4 md:space-y-6">
              <div className="relative">
                <CheckCircle2 className="h-16 w-16 md:h-20 md:w-20 lg:h-24 lg:w-24 text-[#2A9C64] animate-in fade-in zoom-in duration-500" />
              </div>
              <div className="text-center space-y-2 md:space-y-3">
                <h3 className="text-xl md:text-2xl lg:text-3xl font-semibold text-[#2A9C64] dark:text-[#2A9C64]">
                  Welcome!
                </h3>
          ...
          {status === "error" && (
            <div className="flex flex-col items-center justify-center py-8 md:py-12 space-y-4 md:space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full pt-4 md:pt-6">
                ...
                <Button
                  onClick={handleRetry}
                  className="flex-1 h-11 md:h-12 text-base md:text-lg bg-[#2A9C64] hover:bg-[#1E7A4A] text-white transition-all hover:shadow-lg active:scale-[0.98]"
                >
                  Try Again
                </Button>
              </div>
