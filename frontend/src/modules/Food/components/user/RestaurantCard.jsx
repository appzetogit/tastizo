...
const RestaurantImageCarousel = React.memo(({ restaurant, priority = false, backendOrigin = "" }) => {
  ...
  return (
    ...
    >
      ...
      {restaurant.discount && (
        <div className="absolute top-2 left-0 px-2.5 py-1 bg-gradient-to-r from-[#1E7A4A] to-[#2A9C64] text-white text-[10px] sm:text-xs font-black rounded-r-lg shadow-lg uppercase tracking-wider flex items-center gap-1">
    ...
    >
      ...
      <div className="p-3 sm:p-4">
        <div className="flex justify-between items-start gap-2 mb-1.5">
          <h3 className="text-[15px] sm:text-[17px] font-bold text-gray-900 line-clamp-1 group-hover:text-[#2A9C64] transition-colors duration-200 flex-1 tracking-tight">
            {restaurant.name}
          </h3>
        ...
        <div className="flex items-center justify-between pt-2.5 border-t border-gray-100/80">
          <div className="flex items-center gap-1.5 text-gray-600 bg-gray-50 px-2 py-1 rounded-md">
            <Clock className="w-3.5 h-3.5 text-[#2A9C64]" />
          ...
          <div className="flex items-center gap-1 text-gray-600 bg-gray-50 px-2 py-1 rounded-md">
            <IndianRupee className="w-3 h-3 text-[#2A9C64]" />
