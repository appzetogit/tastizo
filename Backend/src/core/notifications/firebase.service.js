...
const getServiceAccountFromEnv = () => {
    ...
    const clientEmail = sanitizeString(config.firebaseClientEmail || process.env.FIREBASE_CLIENT_EMAIL);
    const privateKey = normalizePrivateKey(config.firebasePrivateKey || process.env.FIREBASE_PRIVATE_KEY);
    const projectId = sanitizeString(config.firebaseProjectId || process.env.FIREBASE_PROJECT_ID);
    const privateKeyId = sanitizeString(config.firebasePrivateKeyId || process.env.FIREBASE_PRIVATE_KEY_ID);
    ...
    if (rawJsonParseError) {
        throw new Error(`Firebase service account JSON is invalid: ${rawJsonParseError.message}`);
    }
