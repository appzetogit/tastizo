...
const APPLE_PHONE_PREFIX = "apple_";
const GOOGLE_PHONE_PREFIX = "google_";
...
const sanitizeUserForAuthResponse = (userDoc = {}) => {
  ...
  const safePhone =
    String(rawPhone).startsWith(APPLE_PHONE_PREFIX) ||
    String(rawPhone).startsWith(GOOGLE_PHONE_PREFIX)
      ? ""
      : rawPhone;
  return {
    id,
...
const sanitizeApplePrivateKey = (value) => {
  ...
  try {
    ...
    // Ignore base64 parsing errors and fall back to the raw value.
  }

  return raw.replace(/\\n/g, "\n");
};

const parseAppleUserPayload = (rawUser) => {
...
const buildAppleClientSecret = () => {
  if (
    ...
    throw new ValidationError("Apple Sign-In is not fully configured");
  }

  return jwt.sign(
    {},
    sanitizeApplePrivateKey(config.applePrivateKey),
    {
      algorithm: "ES256",
...
const getAppleSigningKeys = async () => {
  ...
  if (!response.ok) {
    throw new AuthError("Failed to fetch Apple signing keys");
  }

  const data = await response.json();
  const keys = Array.isArray(data?.keys) ? data.keys : [];
  if (keys.length === 0) {
    throw new AuthError("Apple signing keys are unavailable");
  }
...
const verifyAppleIdentityToken = async (identityToken) => {
  ...
  const jwk = appleKeys.find((item) => item.kid === decodedHeader.kid);
  if (!jwk) {
    throw new AuthError("Unable to verify Apple identity token");
  }
...
const exchangeAppleAuthorizationCode = async (code) => {
  if (!code) {
    throw new ValidationError("Apple authorization code is required");
  }
  ...
  const formData = new URLSearchParams({
    ...
    redirect_uri: String(config.appleUserRedirectUri || config.appleRedirectUri || ""),
  });
  ...
  if (!response.ok || payload?.error) {
    logger?.warn?.(
      { status: response.status, appleError: payload?.error, appleErrorDescription: payload?.error_description },
      "Apple token exchange failed",
    );
    throw new AuthError(payload?.error_description || payload?.error || "Apple sign-in failed");
  }

  return payload;
};

const storeUserFcmToken = async (userDoc, fcmToken, platform) => {
  if (!userDoc || !fcmToken) return;
...
const createUserSession = async (userDoc) => {
  ...
  const payload = { userId: user._id.toString(), role: user.role || ROLES.USER };
...
export const loginUserWithApple = async ({
  code,
  identityToken,
  user: rawUser,
  fcmToken = null,
  ...
  if (!config.appleClientId || !(config.appleUserRedirectUri || config.appleRedirectUri)) {
    throw new ValidationError("Apple Sign-In is not configured");
  }

  const tokenPayload = identityToken
    ? { id_token: identityToken }
    : await exchangeAppleAuthorizationCode(code);

  const verifiedIdentity = await verifyAppleIdentityToken(tokenPayload.id_token);
  ...
  const fullName = [appleUser?.name?.firstName, appleUser?.name?.lastName]
    ...
    .trim();
  const email = String(
    verifiedIdentity?.email || appleUser?.email || "",
  )
    .trim()
    .toLowerCase();
  const appleId = String(verifiedIdentity?.sub || "").trim();

  if (!appleId) {
    throw new AuthError("Apple account identifier is missing");
  }

  let userDoc =
    (await FoodUser.findOne({ appleId })) ||
    (email ? await FoodUser.findOne({ email }) : null);

  const isNewUser = !userDoc;
  if (!userDoc) {
    userDoc = await FoodUser.create({
      appleId,
      phone: `${APPLE_PHONE_PREFIX}${appleId}`,
    ...
    if (fullName && !String(userDoc.name || "").trim()) {
      userDoc.name = fullName;
      needsSave = true;
    }
  ...
  await storeUserFcmToken(userDoc, fcmToken, platform);
  const session = await createUserSession(userDoc);
...
export const loginUserWithGoogle = async ({
  ...
  try {
    ...
    decodedToken = await firebaseAdmin.auth().verifyIdToken(String(idToken));
  } catch (error) {
    logger?.warn?.({ err: error }, "Google Firebase token verification failed");
    throw new AuthError("Invalid Google sign-in token");
  }

  const googleId = String(decodedToken?.uid || "").trim();
  const email = String(decodedToken?.email || "").trim().toLowerCase();
  const name = String(decodedToken?.name || "").trim();
  const profileImage = toSafeImageUrl(decodedToken?.picture);

  if (!googleId) {
    throw new AuthError("Google account identifier is missing");
  }

  let userDoc =
    (await FoodUser.findOne({ googleId })) ||
    (email ? await FoodUser.findOne({ email }) : null);
  ...
  if (!userDoc) {
    userDoc = await FoodUser.create({
      googleId,
      phone: `${GOOGLE_PHONE_PREFIX}${googleId}`,
      email: email || undefined,
    ...
    if (name && !String(userDoc.name || "").trim()) {
      userDoc.name = name;
      needsSave = true;
    }
    if (profileImage && !toSafeImageUrl(userDoc.profileImage)) {
